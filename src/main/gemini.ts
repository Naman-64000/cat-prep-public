...
export function setupGemini() {
  return {
    async requestSolution(section: string, imagePaths: string[]) {
      const activeKey = getEffectiveGeminiKey()
      if (!activeKey) {
        throw new Error('Missing GEMINI_API_KEY. Please provide a key in settings or environment.')
      }
      ...
      const sectionPrompt = getPromptForSection(section)
